...
std::pair<torch::Tensor, bool> e27_ProcessFunction (CaptureData ts) {
    ...
    if (count == 20) {
        ...
        if (max_index != e27_global::indicies[indexes]) {
            std::cout << "Mismatch! Expected: " << e27_global::indicies[indexes] << ", Got: " << max_index << "\n";
            ...
            --kills;
            if (kills <= 0)
                e27_global::kill_program.store(true, std::memory_order_release);   
        }
        else {
    ...
    if (first) {
        e27_global::start_time.store(Utils::GetCurrentTime_us(), std::memory_order_release);
        std::cout << "Start time recorded.\n";
    }

    // increment image count
    e27_global::image_count.fetch_add(1, std::memory_order_release);
...
int e27 () {
    ...
    //scheduler.EnableBlendMode();

    // Create a phase mask
    torch::Tensor target_mask = torch::ones({mask_Height, mask_Width}, torch::dtype(torch::kFloat32));
    torch::Tensor initial_mask = torch::zeros({mask_Height, mask_Width}, torch::dtype(torch::kFloat32));
    
    // initial_mask[:50,:50]=1
    initial_mask.slice(0, 0, 50).slice(1, 0, 50) = 1.0f;

    // Get the phase mask using GS algorithm
    auto phase_mask = s4_Utils::GSAlgorithm(target_mask, initial_mask, 100).to(DEVICE);
    ...
    auto Iterate = [&scheduler]->void {
        ...
        scheduler.WaitUntilCameraIsIdle();
        scheduler.Capture();
    ...
    while (!WindowShouldClose()) {
        auto action = torch::ones({20, mask_Height, mask_Width}, torch::dtype(torch::kFloat32).device(DEVICE)) * (0.8f);
        action[e27_global::indicies[index]] = phase_mask;
        scheduler.SetTextureFromTensorTiled(action);
        Iterate();

        // increment frame count
        e27_global::frame_count.fetch_add(1, std::memory_order_release);

        index = (index + 1) % 20;
        
        if (e27_global::kill_program.load(std::memory_order_acquire)) {
    ...
    int64_t total_frames = e27_global::frame_count.load(std::memory_order_acquire);
    int64_t total_images = e27_global::image_count.load(std::memory_order_acquire);
    int64_t start_time   = e27_global::start_time.load(std::memory_order_acquire);
